...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.AUTH_SERVICE_SERVICE_HOST}:${process.env.AUTH_SERVICE_SERVICE_PORT}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTH_SERVICE_SERVICE_HOST}:${process.env.AUTH_SERVICE_SERVICE_PORT}/token/` + hashedPassword + '/' + password
  );
